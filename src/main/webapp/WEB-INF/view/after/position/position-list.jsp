...
<body>
<div class="topfixed">
		<div class="current"><s></s><a href="#">首页</a> >> <a href="#">岗位管理</a> >> <span>岗位列表</span></div>
		
		<div class="btnbox">
			<a href="${pageContext.request.contextPath}/after/position/position-add"><input type="button" value="新 增" class="btn_pink"/></a>
...
<div class="content">
	<form action="${pageContext.request.contextPath}/after/position/position-list" method="post" id="searchForm" style="display: none">
		<dl class="forms" style="padding-bottom: 170px;">
			<dd>
				<label for="search_userName">岗位名称：</label><input id="search_positionName" value="${positionName }" type="text"  name="positionName"  style="margin-left: 4px;"/>
			</dd>
		</dl>
		...
		<table border="0" class="table" id="listTable">
		  ...
		  <tbody>
			  <c:forEach items="${list }" var="obj">
			  		<tr oid="${obj.positionId }" >
					    <td class="center"><input type="checkbox" value="${obj.positionId }" name="obj" /></td>
					    <td>${fn:escapeXml(obj.positionName)  }</td>
					    <td>
	...
	<div class="pages">
    	<pageTag:pagin totalPage="${ page.totalPages}" currentPage="${page.pageNo }" uri="${pageContext.request.contextPath}/after/position/position-list"></pageTag:pagin>
    </div>
</div>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$('#delete').live('click',function(){
			...
			$.get('${pageContext.request.contextPath}/after/position/position-deleteBatch/'+ids.join(","),function(resp){
		...
		$("#listTable tbody tr td:not(.center)").live('click',function(){
			...
			var $id = $tr.attr('oid');
			location.href = '${pageContext.request.contextPath}/after/position/position-edit/'+$id;
		});
