...
<body>
<div class="topfixed">
		<div class="current"><s></s><a href="#">企业管理</a> >><span>企业产品管理</span></div>
		
		<div class="btnbox">
			<a href="${pageContext.request.contextPath}/after/product/add">
...
<div class="content">
	<form action="${pageContext.request.contextPath}/after/product/list" method="post" id="userSearchForm" style="display: none">
		...
		<table border="0" class="table" id="userListTable">
		  <thead>
		 	<tr>
			    ...
			    <c:if test="${sessionScope.SESSIONUSER.type eq 0}"><th width="50">企业名称</th></c:if>
			    <th width="50">产品图片</th>
			    <th width="50">产品名称</th>
			    <th width="80">产品介绍</th>
			   </tr>
		  ...
		  <tbody>
		    <c:forEach items="${list }" var="organ">
			  ...
			  <td style="word-break:break-all" class="center">
			    ...
			    <td style="word-break:break-all"><img src="${organ.logo}" width="200" height="100"/></td>
			    <td style="word-break:break-all">${organ.name}</td>
			    <td style="word-break:break-all">${organ.introduce}</td>
			  </tr>
		    </c:forEach>
		  </tbody>
		</table>
	...
	<div class="pages">
    	...
    	<pageTag:pagin totalPage="${page.totalPages}" currentPage="${page.pageNo }" uri="${pageContext.request.contextPath}/after/product/list"></pageTag:pagin>
    </div>
</div>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$('#delete').live('click',function(){
			...
			if (ids.length <= 0)
				...
				layer.confirm("您确定要删除勾选的数据吗？",function(index) {
					$.ajax({
						url:'${pageContext.request.contextPath}/after/product/del?id='+ids.join("`"),
						dataType:'json',
						type:'get',
		...
		$("#userListTable tbody tr td:not(.center)").live('click',function(){
			...
			location = '${pageContext.request.contextPath}/after/product/edit/'+$id;
		});
	});
