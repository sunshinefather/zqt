...
<body>
<div class="topfixed">
	<div class="current"><s></s><a href="${pageContext.request.contextPath}/after/content/gonggao">公告管理</a> >> <span>编辑</span></div>
	<div class="btnbox">
...
<div class="content">
     <dl class="forms">
         ...
         <input id="categoryId" name="categoryId" value="4" type="hidden"/>
         <dd><label for="title"><span>*</span>公告标题：</label><input id="title" name="title" value="${obj.title }" type="text" require="require"/></dd>
        <dd><label for="str1"><span>*</span>公告时间：</label><input id="str1" name="str1" value="${obj.str1 }" type="text" style="width: 150px;" class="Wdate" onclick="WdatePicker({autoPickDate:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" require="require"/></dd>
        <dd><label for="content"><span>*</span>公告内容：</label>
        <textarea  id="content" name="content" style="width:300px; height:80px;position:relative;left:0px;">${obj.content}</textarea>
        </dd>
		...
		f.find("input,select:enabled").each(function(index, element) {
			...
			$.ajax({
				...
				success:function(resp){
					if (resp.status == 1){
						location="${pageContext.request.contextPath}/after/content/gonggao";
					}else{
						layer.alert("保存失败！");
					}
				}
