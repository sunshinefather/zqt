...
<div class="content">
     <dl class="forms">
        ...
        <dd id="org" <c:if test="${obj.type == '0'}">style="display: none;"</c:if>>
        ...
        <dd id="ugg">
        	...
        	<input id="groups" readonly="readonly"  type="text" value='<c:forEach items="${obj.groups }" varStatus="i" var="group"><c:if test="${i.count > 1 }">,</c:if>${group.groupName }</c:forEach>' <c:if test="${obj.type != '0'}">require="require"</c:if> title="用户组" />
	        	<span id="groupsSpan">
...
<script type="text/javascript">
	...
	$(function(){
		...
		$('#type').live('change',function(){
			var value = $(this).val(),_org=$("#org"),_groups=$("#groups"),
				 _sel = $("select[name='extUser.orgId']");
			if (value == '1'){
				_org.show();
				_sel.attr("require","require").attr("title","所属企业");
				_sel.removeAttr("disabled");
				_groups.attr("require","require")
